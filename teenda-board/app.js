(()=>{
    ...
    function render(){
      ...
      app.classList.toggle('editing',editing);editToggle.setAttribute('aria-pressed',String(editing));editToggle.textContent=editing?'完了':'編集';grid.textContent='';
      ...
      for(const item of messages){
        ...
        const button=document.createElement('button');button.className='message-button';button.type='button';button.textContent=item.label||item.text;button.addEventListener('click',()=>{if(!editing)showMessage(item)});card.appendChild(button);
        if(item.custom){const actions=document.createElement('div');actions.className='edit-actions';const edit=document.createElement('button');edit.type='button';edit.className='mini-action';edit.setAttribute('aria-label','編集');edit.textContent='✎';edit.addEventListener('click',()=>openSaveForm(item));const del=document.createElement('button');del.type='button';del.className='mini-action delete';del.setAttribute('aria-label','削除');del.textContent='×';del.addEventListener('click',()=>removeCustom(item.id));actions.append(edit,del);card.appendChild(actions)}
    ...
    editToggle.addEventListener('click',()=>{editing=!editing;render()});addButton.addEventListener('click',()=>openSaveForm());instantButton.addEventListener('click',openInstantForm);closeModal.addEventListener('click',closeForm);modal.addEventListener('click',event=>{if(event.target===modal)closeForm()});textInput.addEventListener('input',updateCount);backButton.addEventListener('click',hideMessage);
